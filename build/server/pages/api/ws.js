...
module.exports = __webpack_require__("hol+");
...
const WebSocket = __webpack_require__("kEOu");
  ...
  if (!res.socket.server.ws) {
    ...
    setInterval(() => {
      for (const [room, roomData] of Object.entries(rooms)) {
        ...
        if ((_roomData$game = roomData.game) === null || _roomData$game === void 0 ? void 0 : _roomData$game.tick) {
          if (roomData.game.tick) {
            if (moreThanOneHourAgo(new Date(roomData.game.tick))) {
              ...
              wss.broadcast(room, JSON.stringify({
                action: "data",
                data: {}
              }));
              wss.broadcast(room, JSON.stringify({
      ...
      ws.on("message", function incoming(message) {
        try {
          ...
          if (message.action === "load_game") {
            ...
            game.round = 0;
            game.title = true;
            game.rounds = message.data.rounds; // clone the final round so we can store data about the second final round
            ...
            let id = registerPlayer(roomCode, true, {}, ws); // send back details to client
            ...
            if (rooms[room_code]) {
              if (rooms[room_code].game.registeredPlayers[user_id]) {
                console.debug("user session get_back_in:", room_code, user_id, team); // set the new websocket connection
            ...
            try {
              ...
              console.debug("buzzer ready: ", id); // get inital latency, client pongs on registered
